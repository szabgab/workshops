...
def create_thumbnail(episode, show=False):
    ...
    png_filepath = os.path.join(root, 'images', 'dancer', png_filename)
    #print(png_filepath)
    ...
    add_text(
        draw = draw,
        text = f"Perl Dancer  -  Part {episode}",
        rtl  = False,
        size = 80,
        xy   = (400, 140),
    )
...
#    )
    ...
    embed_image(img=img, filename='dancer.png', size=(isize, isize), box=(10, 10), mask=True)
